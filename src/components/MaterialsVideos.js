...
const VideoCardT = ({ video }) => {
    ...
    const handleDelete = () => {
        ...
        fetch(`${apiBaseUrl}/api/deletemv`, {
            ...
            body: JSON.stringify({ id: handleid, mvpath: ipath }),
        })
            .then(response => {
                if (!response.ok) {
...
const MaterialsVideos = () => {
    ...
    const [pageSize] = useState(12); // 每页显示的图片数,4的倍数
    ...
    return (
        <div className='content' >
            ...
            <div className='videos'>
                {currentDocuments.map((video, index) => (
                    <VideoCardT
                        key={index}
                        video={video}
                    />
...
export default MaterialsVideos;